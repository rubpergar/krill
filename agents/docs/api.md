...
| GET | `/products` | Product list page (ordered by name). Accepts `?q=` (search name/SKU), `?category=` (filter by category ID), `?low_stock=1` (filter by stock ≤ min_stock), and `?success=created|updated|toggled|stock_updated` for flash messages |
...
| GET | `/products/:id/stock` | Stock adjustment form. Returns 404 if `:id` does not exist |
| POST | `/products/:id/stock` | Adjust stock (accepts `operation=add|remove` and `quantity`). Returns 200 with error on validation failure. Returns 404 if `:id` does not exist |
