...
| Invalid approval | Put a task without `approved_at` in `current/` and invoke active-task commands | `/plan`, `/implement`, `/audit-tests`, `/review-task`, and `/closeout` stop instead of silently treating the move as approval |
